 ...
 * file before streaming. Use {@code --no-validate} ({@code -V}) to skip
 * validation entirely. All progress messages are written to stderr so they do
 ...
 * deletes the output, and exits with code 1. Use {@code --no-validate} to skip
 * validation.
 *
 * <br>
...
public class ConvTab2Xdxf {
	...
	private static final Set<String> ALLOWED_TAGS = Set.of("p", "b", "i", "em", "strong", "ol", "ul", "li", "sub",
			"sup", "br", "a", "div", "span", "table", "tr", "td", "th");
	...
	public static void main(String[] args) throws Exception {
		...
		if (configPath != null) {
			...
			try (var r = new java.io.InputStreamReader(new FileInputStream(cp.toFile()), StandardCharsets.UTF_8)) {
				config.load(r);
			}
		}
	...
	static void validateXml(Path outFile) throws Exception {
		try {
			...
			err(String.format("XML validation FAILED at line %d, column %d: %s", e.getLineNumber(), e.getColumnNumber(),
					e.getMessage()));
			try {
	...
	static String cleanHtml(String html) {
		html = html.replace("&nbsp;", " ").replace("&emsp;", " ").replace("&ensp;", " ").replace("&thinsp;", " ")
				.replace("&mdash;", "—").replace("&ndash;", "–").replace("&laquo;", "«").replace("&raquo;", "»")
				.replace("&ldquo;", "\u201C").replace("&rdquo;", "\u201D").replace("&lsquo;", "\u2018")
				.replace("&rsquo;", "\u2019").replace("&hellip;", "…").replace("&bull;", "•").replace("&middot;", "·")
				.replace("&times;", "×").replace("&divide;", "÷").replace("&deg;", "°").replace("&acute;", "´")
				.replace("&cedil;", "¸");
